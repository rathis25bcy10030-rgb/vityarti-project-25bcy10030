...
3)Run the Application: Ensure both project.py (database logic) and project2.py (GUI/main application) are in the same directory. Run the main GUI file: python project2.py
                                                                                                                                                          
File Name,Description,Module
project2.py,"Main Application (GUI & Frontend): Contains the BudgetApp class, which handles the tkinter layout, user interactions, and calls the database functions.",Module 2 & 3
project.py,"Database Manager (Backend Logic): Contains all functions for interacting with the SQLite database (initialize, add_expense, fetch_expenses, update, get_budget).",Module 1
tracker.db,Automatically created SQLite database file used to store the budget and expense data.,Data
...
add_expense(): Inserts a new expense record into the expenses table. (Note: The current implementation uses incorrect string formatting for the SQL query, which should be corrected to use parameter substitution to prevent SQL Injection.)
